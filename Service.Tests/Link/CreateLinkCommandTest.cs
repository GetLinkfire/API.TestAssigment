...
namespace Service.Tests.Link
{
    ...
    public class CreateLinkCommandTest
    {
        ...
        public void Execute_WithoutCode_Music()
        {
            ...
            /*
             * Fixed Unit Test. For Media Type Music it takes the IsActive Value given in the Argument, so it was being set to false
             * but assert is expecting True
             * If there's a business logic behind the behavior difference between Media Type Music and Ticket for IsActive value, then
             * we would need to make sure to validate the 'True' value. Probably creating another UT.
             * But if there's no business logic. The way to go would be unify the Behavior for IsActive inside the CreateLinkCommand.Execute             * 
             */
        ...
        public void Execute_NewCode_Invalid()
        {
            ...
            /*
             * Fixed Unit Test. Previoulsy it was using "/" as a Separator. Diferent from Path.DirectorySeparator, which is
             * The Separator used to Split the Directory Path in LinkHelper.cs Line 41.            
             */

            _storageService.Expect(x => x.GetFileList(Arg<string>.Is.Equal($"{domain.Name}"), Arg<string>.Is.Equal("code")))
                .Return(new List<string>() { string.Format("{0}{1}{2}", domain.Name, System.IO.Path.DirectorySeparatorChar, "code") });
